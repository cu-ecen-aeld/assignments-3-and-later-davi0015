...
cd ${OUTDIR}
OUTDIR=$(pwd)
echo "The absolute path is ${OUTDIR}"
...
libc=$(dirname $(which ${CROSS_COMPILE}gcc))/../${CROSS_COMPILE::-1}/libc
cp ${libc}/lib/ld-linux-aarch64.so.1 lib/
cp ${libc}/lib64/libm.so.6 lib64/
cp ${libc}/lib64/libresolv.so.2 lib64/
cp ${libc}/lib64/libc.so.6 lib64/
